...
def gitlab_safe_get(url, token=None):
    headers = {}
    if token:
...
def get_gitlab_commits(owner, repo, token, start_date):
    ...
    repo_commits = gitlab_safe_get(repo_commits_url, token)
    ...
    for commit in repo_commits:
        ...
        commit_detail_url = f"{Config.GITLAB_BASE_API}/projects/{encoded}/repository/commits/{commit_id}"
        commit_detail = gitlab_safe_get(commit_detail_url, token)
